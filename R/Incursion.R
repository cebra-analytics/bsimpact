...
#' invasive species presence or density, area of occupancy, or incursion
#' likelihoods across a defined region for impact analysis, and transforms the
#' incursion values for impact calculations.
#'
#' @param x A \code{terra::SpatRaster}, \code{raster::RasterLayer}, or numeric
#'   vector defining the incursion presence, density, area of occupancy (metres
#'   squared), or probabilities at each spatial location.
#' @param region A \code{Region} or inherited class object representing the
#'   spatial region (template) for impact analysis.
#' @param type One of \code{"presence"}, \code{"density"}, \code{"area"}, or
#'   \code{"prob"} to indicate if the values in \code{x} represent incursion
#'   presence, density, area of occupancy, or probabilities respectively.
...
Incursion <- function(x,
                      region = NULL,
                      type = c("presence", "density", "area", "prob"),
                      multiplier = 1,
                      threshold = 0, ...) {
  UseMethod("Incursion")
}
...
Incursion.default <- function(x,
  ...
  self$get_impact_incursion <- function() {
    ...
    # Check for recovery delay and dynamic multiplier
    recovery_delay <- attr(x, "recovery_delay")
    dynamic_mult <- attr(x, "dynamic_mult")
    ...
    # Re-attach recovery delay and dynamic multiplier
    attr(x, "recovery_delay") <- recovery_delay
    attr(x, "dynamic_mult") <- dynamic_mult
